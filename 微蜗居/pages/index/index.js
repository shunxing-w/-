...
Page({
  ...
  get_productlist(shop_id,cate_id,sessionid) {
    ...
    wx.request({
      url: 'https://www.weiwoju.com/WechatApplet/getProductList',
      method: 'GET',
      data: {
        cate_ids: cate_id,
        shop_id: shop_id,
  ...
  achieve: function () {
    ...
    if(wx.openSetting){
      ...
      wx.showModal({
        title: '提示',
        content: "当前微信版本过低，无法正常使用，请升级微信版本",
        success: function (res) {
    ...
    wx.getLocation({
      ...
      fail: function () {
		...
		var enable = this.data.enable;
		wx.request({
			url: 'https://www.weiwoju.com/WechatApplet/shoppingCar?shop_id=' +shop_id,
			method: 'GET',
			data: {
				session_rd: session_rd,
        ver_no: app.data.edition
		...
		if (wx.getStorageSync('session_rd') == false || wx.getStorageSync('session_rd') == '') {
			wx.login({
				success: function (res) {//登录成功
                    ...
                    that.get_shop(options.id,session_rd);
				...
				fail: function (res) {	console.log('登录失败' + res);}
			});
		} else {
